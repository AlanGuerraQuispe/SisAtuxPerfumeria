...
public class CProductoLocal extends JAbstractController implements Serializable{
    ...
    public boolean guardarRegistroDescuento(ProductoLocal productoLocal,String coDescuento,String inGenero) throws SQLException{
        ProductoLocal ap = productoLocal;
        int gravado = 0;
        ...
        gravado = this.agregarRegistroPs(DescuentoProducto.nt, DescuentoProducto.FULL_NOM_CAMPOS, valores);
    ...
    public int actualizarProductoDescuento(ProductoLocal pl,String coDescuento,String inGenero) throws SQLException {

        DescuentoProducto ap = new DescuentoProducto(pl.getCoCompania(),pl.getCoLocal(),pl.getCoProducto(),pl.getNuRevisionProducto(),inGenero,
                                                    coDescuento,AtuxSearch.getFechaHoraTimestamp(),AtuxSearch.getFechaHoraTimestamp(),pl.getEsProdLocal(),pl.getIdModProdLocal(),AtuxSearch.getFechaHoraTimestamp());

        int gravado = 0;

        Object[] valores = {ap.getInGenero(),
                ap.getCoDescuento(),
                ap.getFeFinVigencia(),
                ap.getFeFinVigencia(),
                ap.getEsDescuentoProducto(),
                AtuxVariables.vIdUsuario,
                ap.getFeModDescuentoProducto()
        };

        gravado = this.actualizarRegistroPs(DescuentoProducto.nt, this.adjuntarSimbolo(generarArrayAString(DescuentoProducto.CAMPOS_NO_ID), ",", "?")+Ex.WHERE+ unirColumnasValores(DescuentoProducto.COLUMNA_PK, ap.getPrimaryKey()) , valores);
...
//            query.append("       (SELECT SUM(PRO.VA_PRECIO_PUBLICO) FROM  LGTM_PRODUCTO PRO, LGTR_PRODUCTO_INSUMO INS WHERE PRO.CO_COMPANIA = INS.CO_COMPANIA AND PRO.CO_PRODUCTO = INS.CO_PRODUCTO_INSUMO AND INS.CO_PRODUCTO = T1.CO_PRODUCTO)  VA_VENTA, ");
...
//            query.append("       (SELECT SUM(PRO.VA_PRECIO_PUBLICO) FROM  LGTM_PRODUCTO PRO, LGTR_PRODUCTO_INSUMO INS WHERE PRO.CO_COMPANIA = INS.CO_COMPANIA AND PRO.CO_PRODUCTO = INS.CO_PRODUCTO_INSUMO AND INS.CO_PRODUCTO = T1.CO_PRODUCTO) PRECIO_PUB, ");
            ...
            query.append("        AND T1.TI_MATERIAL_SAP='PROD'\n");//todo para productos terminados
    ...
    public ArrayList getInsumosProductos(ProductoLocal prodLocal){
        ...
        if (!prodLocal.getInInsumos()){
            try {
                ...
                query.append("AND    INS.ES_PRODUCTO_INSUMO = 'A' \n");
                ...
                query.append("AND    INS.CO_COMPANIA = '").append(prodLocal.getCoCompania()).append("'");
                query.append("AND    INS.ES_PRODUCTO_INSUMO = 'A' \n");
                query.append("AND    INS.CO_PRODUCTO IN \n");
                query.append("       (SELECT INS.CO_PRODUCTO_INSUMO \n");
                query.append("       FROM    LGTM_PRODUCTO PRO,  \n");
                query.append("               LGTR_PRODUCTO_INSUMO INS \n");
                query.append("       WHERE   PRO.CO_COMPANIA        = INS.CO_COMPANIA \n");
                query.append("       AND     PRO.CO_PRODUCTO        = INS.CO_PRODUCTO_INSUMO \n");
...
//                        sm.setProducto(new CProducto().getRegistroPorPk(new Object[]{rs.getString(1), rs.getString(3), rs.getString(4)}));
...
//                        prod.setCoImpuesto_1(rs.getString("CO_IMPUESTO_1"));
                    prodLocalAux.setProducto(new CProducto().getRegistroPorPk(new Object[]{rs.getString("CO_COMPANIA"), rs.getString("CO_PRODUCTO_INSUMO"), rs.getString("NU_REVISION_PRODUCTO")}));
                    ...
                    prodLocalAux.setVaPrecioPublico(rs.getDouble("VA_PRECIO_PUBLICO"));
                    prodLocalAux.setVaVenta(rs.getDouble("VA_PRECIO_PUBLICO"));
                    prodLocalAux.setInImpresion(rs.getString("IN_IMPRESION"));
    ...
    public boolean guardarDescuento(JAbstractModelBD mdl) throws SQLException{
        ProductoLocal ap = (ProductoLocal)mdl;
        int gravado = 0;
        String campos = ProductoLocal.FULL_MAESTROS.toString();
