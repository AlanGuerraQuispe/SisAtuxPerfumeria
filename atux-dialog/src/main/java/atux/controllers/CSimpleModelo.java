...
public class CSimpleModelo extends JAbstractController implements Serializable{
    ...
    public ArrayList getRegistrosDeDescuento() {
        ArrayList<Descuento> rgs = new ArrayList<Descuento>();
        try {

            //public ResultSet getRegistros(String nombreTabla, String[] campos, String[] columnaId, Object[] id,String[] columOrder) {
            //rs =  this.getRegistros(CajaPago.nt,CajaPago.FULL_NOM_CAMPOS,new String[]{CajaPago.COLUMNA_ACTIVO},new Object[]{"A"},new String[]{"NU_CAJA"});
            rs = this.getRegistros(Descuento.nt,Descuento.FULL_NOM_CAMPOS,null,null,new String[]{"CO_DESCUENTO"});

            while(rs.next())
            {
                descuento = new Descuento();
                descuento.setPrimaryKey(new String[]{rs.getString("CO_COMPANIA"),rs.getString("CO_DESCUENTO")});
                descuento.setCoCompania(rs.getString("CO_COMPANIA"));
                descuento.setCoDescuento(rs.getString("CO_DESCUENTO"));
                descuento.setPcDescuento(rs.getDouble("PC_DESCUENTO"));
                descuento.setDeCortaDescuento(rs.getString("DE_CORTA_DESCUENTO"));
                descuento.setDeDescuento(rs.getString("DE_DESCUENTO"));
                descuento.setEsDescuento(rs.getString("ES_DESCUENTO"));
                descuento.setIdCreaDescuento(rs.getString("ID_CREA_DESCUENTO"));
                descuento.setFeCreaDescuento(rs.getDate("FE_CREA_DESCUENTO"));
    ...
    public ArrayList getRegistrosTipoProducto() {
        ArrayList<TipoProducto> rgs = new ArrayList<TipoProducto>();
        try {

            rs = this.getRegistros(TipoProducto.nt,TipoProducto.FULL_NOM_CAMPOS);

            while(rs.next())
            {
                tiProducto = new TipoProducto();
                tiProducto.setPrimaryKey(new String[]{rs.getString("NU_ORDEN_FILA")});
                tiProducto.setNuOrdenFila(rs.getInt("NU_ORDEN_FILA"));
                tiProducto.setTiProducto(rs.getString("TI_PRODUCTO"));
                tiProducto.setDeTipoProducto(rs.getString("DESCRIPCION_TIPO"));
                tiProducto.setEsTipoProducto(rs.getString("ES_TIPO_PRODUCTO"));
                tiProducto.setIdCreaTipoProducto(rs.getString("ID_CREA_TIPO_PROD"));
                tiProducto.setFeCreaTipoProducto(rs.getDate("FE_CREA_TIPO_PROD"));
    ...
    public DescuentoProducto getDescuentoProducto(String coCompania, String coLocal, String coProducto, String nuRevisionProducto)
    {
        try {
            StringBuffer query = new StringBuffer();
            query.append("SELECT IN_GENERO ,  \n");
            query.append("       CO_DESCUENTO  \n");
            query.append("FROM   VTTX_DESCUENTO_PRODUCTO \n");
            query.append("WHERE CO_COMPANIA='" +coCompania+ "' \n");
            query.append("AND CO_LOCAL='" +coLocal+ "' \n");
            query.append("AND CO_PRODUCTO='" +coProducto+ "' \n");
            query.append("AND NU_REVISION_PRODUCTO='" +nuRevisionProducto+ "' \n");
            ...
            while(rs.next())
            {
                ...
                descuentoProducto.setPrimaryKey(new String[]{coCompania,coLocal,coProducto,nuRevisionProducto});
                descuentoProducto.setCoCompania(coCompania);
                descuentoProducto.setCoLocal(coLocal);
                descuentoProducto.setCoProducto(coProducto);
                descuentoProducto.setNuRevisionProducto(nuRevisionProducto);
                descuentoProducto.setInGenero(rs.getString("IN_GENERO"));
                descuentoProducto.setCoDescuento(rs.getString("CO_DESCUENTO"));
            }
        } catch (SQLException ex) {
            ex.printStackTrace();
        }
