...
public class TestigoTicketBuilder extends TestigoBuilder {

    private int cantidadProductos = 0;
    private PedidoVenta pedido;
    
    public TestigoTicketBuilder(PedidoVenta pedido) {
        this.pedido = pedido;
    }
    ...
    public void printHeader(AtuxPrint vPrint) {
        vPrint.activateCondensed();
        vPrint.printLine(AtuxPrintUtility.alinearIzquierda(AtuxVariables.vDescripcionCompania, 21) + AtuxPrintUtility.llenarBlancos(9) + AtuxPrintUtility.alinearIzquierda("Ped : " + pedido.getNuPedidoDiario(), 12), true);
        ...
        vPrint.activateCondensed();
        vPrint.printLine("Fec:" + getFechaActual() + AtuxPrintUtility.llenarBlancos(1) + "Correl: " + pedido.getNuPedido(), true);
        vPrint.printLine(AtuxPrintUtility.alinearIzquierda("Cliente :" + pedido.getNoImpresoCliente(), 42), true);
        vPrint.deactivateCondensed();
    }
    ...
    public void printDetail(AtuxPrint vPrint) {
        try {
            for (int i = 0; i < pedido.getCaItem(); i++) {
                ...
                String lab      = ""; // todo aguerra no laboratorio
                // ((DetallePedidoVenta)pedido.getDetallePedidoVenta().get(i)).getProdLocal().getProducto().getLaboratorio().getDeLaboratorio();
    ...
    public void printFooter(AtuxPrint vPrint) {
        ...
        if (AtuxVariables.vCortaTicket.equals("S")) {
            vPrint.printLine((char) (27) + "d2", true);
        }
        vPrint.deactivateCondensed();
    }
}
