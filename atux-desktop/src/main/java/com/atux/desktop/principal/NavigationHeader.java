...
public class NavigationHeader extends JComponent {
    ...
    NavigationHeader() {
        ...
        add(Box.createRigidArea(new Dimension(120, 2)));
    ...
    private void removeLinksAbove(final PathButton pathButton) {
        while (buttonStack.size() > 1 && !buttonStack.peek().equals(pathButton)) {
            remove(buttonStack.pop());
        }
...
//            TextLayout layout = new TextLayout(getText(), pathFont, context);
            ...
            as.addAttribute(TextAttribute.FONT, pathFont);
            if (this != buttonStack.peek()){
                as.addAttribute(TextAttribute.UNDERLINE, TextAttribute.UNDERLINE_ON, 0, getText().length());
            }
//            as.addAttribute(TextAttribute.STRIKETHROUGH, TextAttribute.STRIKETHROUGH_ON, 18, 25);

            TextLayout layout = new TextLayout(as.getIterator(), context);
...
//                    break;
                default:
                    TransitionManager.activateMenu(pathButton.getText());
                    removeLinksAbove(pathButton);
                    break;
            }
        }
