...
//		setBorder(BorderFactory.createLineBorder(focusColor));
		setBorder(BorderFactory.createLineBorder(LookAndFeelManager.COLOR_BKG_TITLE));
		reflectFocusedDate();
	}
	...
	class DateLabel extends JLabel {
		...
		private void udapteBorder() {
			if (isFocused() && isEnabled()) {
                setBorder(new LineBorder(LookAndFeelManager.COLOR_BKG_SELECTED));
...
//				setBorder(BorderFactory.createEmptyBorder(1, 1, 1, 1));
		...
		private void updateBg() {
			if (isSelected()) {
				setOpaque(true);
				setBackground(isEnabled() ? LookAndFeelManager.COLOR_BKG_SELECTED : selBgDis);
			} else {
                setBackground(isEnabled() ? LookAndFeelManager.COLOR_BKG_TITLE : selBgDis);
                setOpaque(false);
            }
